...
const shared_1 = require("@izerwaren/shared");
const config_1 = __importDefault(require("../lib/config"));
// Helper function to find related products
async function findRelatedProducts(product, limit = 6) {
    const relatedProducts = [];
    try {
        // 1. Find products in the same category
        const sameCategoryProducts = await database_1.prisma.product.findMany({
            where: {
        ...
        if (relatedProducts.length < limit && product.vendor) {
            const sameVendorProducts = await database_1.prisma.product.findMany({
                where: {
        ...
        if (relatedProducts.length < limit && product.technicalSpecs?.length > 0) {
            ...
            const specCategories = [...new Set(specs.map((spec) => spec.category).filter(Boolean))];
            if (specCategories.length > 0) {
                const similarSpecProducts = await database_1.prisma.product.findMany({
                    where: {
                        technicalSpecs: {
                            some: {
...
const productQuerySchema = zod_1.z.object({
    ...
    sku: zod_1.z.string().optional(), // New: SKU filtering for product detail pages
    category: zod_1.z.string().optional(),
    ownerCategory: zod_1.z.string().optional(), // Owner's category filtering
    categoryName: zod_1.z.string().optional(), // Multiple sub-category filtering (comma-separated)
    subCategory: zod_1.z.string().optional(), // Backward compatibility
    ...
    maxPrice: zod_1.z
        ...
        .optional()
        .transform(val => (val ? parseFloat(val) : undefined)),
    availability: zod_1.z.string().optional(), // Comma-separated availability statuses
    materials: zod_1.z.string().optional(), // Comma-separated materials
    brands: zod_1.z.string().optional(), // Comma-separated brands
    technicalSpecs: zod_1.z.string().optional(), // JSON string of technical spec filters
    hasVariants: zod_1.z
        .string()
        .optional()
...
const productSkuParamsSchema = zod_1.z.object({
...
router.get('/categories', async (req, res) => {
    try {
        const categories = shared_1.categoryMappingService.getAllMappedCategoryDetails();
        const summary = shared_1.categoryMappingService.getMappingSummary();
        res.json({
            data: categories,
            summary,
        });
...
router.get('/filter-options', async (req, res) => {
    try {
        ...
        const technicalSpecs = await database_1.prisma.technicalSpecification.findMany({
            where: {
                isSearchable: true,
            },
            select: {
        ...
        // Get price range
        const priceRange = await database_1.prisma.product.aggregate({
            _min: { price: true },
            _max: { price: true },
            where: {
                price: { not: null },
        ...
        const availabilityOptions = await database_1.prisma.product.findMany({
            where: {
        ...
        res.json({
            data: {
                ...
                materials,
                brands,
                categories: shared_1.categoryMappingService.getAllMappedCategoryDetails(),
            },
        });
    }
    catch (error) {
 ...
 * GET /api/v1/products
 */
router.get('/', (0, validation_1.validateRequest)({ query: productQuerySchema }), async (req, res) => {
    try {
        const parsedQuery = productQuerySchema.parse(req.query);
        const { page, limit, search, sku, category, ownerCategory, categoryName, subCategory, status, minPrice, maxPrice, availability, materials, brands, technicalSpecs, hasVariants, hasPdf } = parsedQuery;
        ...
        if (categoryName) {
            ...
            const dbCategories = shared_1.categoryMappingService.getDbCategoriesForOwner(ownerCategory);
...
router.get('/variants', async (req, res) => {
    try {
        ...
        const variantType = req.query.variantType;
        ...
        res.json({
            ...
            meta: {
                ...
                variantCounts: Object.fromEntries(Object.entries(variantTypeMapping).map(([type, skus]) => [type, skus.length])),
            },
        });
    }
    catch (error) {
 ...
 */
router.get('/:id', (0, validation_1.validateRequest)({ params: productParamsSchema }), async (req, res) => {
    try {
        const { id } = req.params;
 ...
 */
router.get('/sku/:sku', (0, validation_1.validateRequest)({ params: productSkuParamsSchema }), async (req, res) => {
    try {
        const { sku } = req.params;
        ...
        const technicalSpecsByCategory = product.technicalSpecs?.reduce((acc, spec) => {
        ...
        const enhancedProduct = {
            ...
            availabilityDetails: {
                ...
                quantityAvailable: product.shopifyVariants?.reduce((sum, variant) => sum + (variant.inventoryQty || 0), 0) || 0,
 ...
 */
router.get('/:id/variants', (0, validation_1.validateRequest)({ params: productParamsSchema }), async (req, res) => {
    try {
        const { id } = req.params;
 ...
 */
router.get('/:id/images', (0, validation_1.validateRequest)({ params: productParamsSchema }), async (req, res) => {
    try {
        const { id } = req.params;
        const images = await database_1.prisma.productImage.findMany({
            where: { productId: id },
            orderBy: [{ isPrimary: 'desc' }, { imageOrder: 'asc' }],
        });
 ...
 */
router.get('/:id/specifications', (0, validation_1.validateRequest)({ params: productParamsSchema }), async (req, res) => {
    try {
        const { id } = req.params;
        const specifications = await database_1.prisma.technicalSpecification.findMany({
            where: { productId: id },
            orderBy: { name: 'asc' },
        });
 ...
 */
router.get('/:id/inventory', (0, validation_1.validateRequest)({ params: productParamsSchema }), async (req, res) => {
    try {
        const { id } = req.params;
