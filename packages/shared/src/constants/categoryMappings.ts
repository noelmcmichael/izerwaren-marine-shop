import { CategoryMapping, OwnerCategoryName } from '../types/Category';
...
export const CATEGORY_MAPPINGS: CategoryMapping[] = [
  ...
  },
  {
    ...
    productCount: 76,
    description:
      'Decorative door levers, escutcheon plates, and rose trim for marine applications',
  },
  {
  ...
  },
  {
    ...
    productCount: 87,
    description:
      'Decorative and artistic hardware designs for luxury marine applications',
  },
  {
 ...
 */
export const getMappedOwnerCategories = (): OwnerCategoryName[] => {
  return CATEGORY_MAPPINGS.map(mapping => mapping.ownerCategory as OwnerCategoryName);
};

/**
 * Get database categories for a given owner category
 */
export const getDbCategoriesForOwner = (ownerCategory: string): string[] => {
  const mapping = CATEGORY_MAPPINGS.find(m => m.ownerCategory === ownerCategory);
 ...
 */
export const mapDbCategoryToOwner = (dbCategory: string): string | null => {
 ...
 */
export const getProductCountForOwnerCategory = (ownerCategory: string): number => {
  const mapping = CATEGORY_MAPPINGS.find(m => m.ownerCategory === ownerCategory);
 ...
 */
export const ALL_OWNER_CATEGORIES: OwnerCategoryName[] = [
  'MARINE LOCKS',
