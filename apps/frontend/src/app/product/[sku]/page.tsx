...
// Import Task 10 Enhanced Specification System
import { SpecificationDisplay } from '../../../components/specifications';

// Import Task 9.5 Comparison and Recently Viewed Features
...
export default function EnhancedProductDetailPage() {
  ...
  // Task 9.5: Comparison and Recently Viewed Features
  const { products: comparisonProducts, addProduct: addToComparison, removeProduct: removeFromComparison } = useComparison();
  ...
  useEffect(() => {
    ...
    const fetchProduct = async () => {
      ...
      try {
        ...
        // Task 9.5: Add to recently viewed
        if (result.data) {
          addRecentlyViewed({
            id: result.data.id,
            title: result.data.title,
            sku: result.data.sku,
            price: result.data.price?.toString() || '0.00',
            vendor: result.data.vendor || '',
            imageUrl: result.data.images?.[0]?.imageUrl || result.data.images?.[0]?.localPath || '/images/placeholder.jpg'
          });
        }
      } catch (err) {
                ...
                <div className='space-y-4'>
                  ...
                  <div className='flex items-center space-x-4'>
                    ...
                    <button
                      onClick={() => {
                        if (comparisonProducts.some(p => p.id === product.id)) {
                          ...
                          addToComparison({
                            ...
                            imageUrl: product.images?.[0]?.imageUrl || product.images?.[0]?.localPath || '/images/placeholder.jpg'
                          });
                        }
                      }}
                      ...
                      title={comparisonProducts.some(p => p.id === product.id) ? 'Remove from comparison' : 'Add to comparison'}
                    >
                      <BarChart3 className='h-4 w-4 mr-1' />
                      {comparisonProducts.some(p => p.id === product.id) ? 'Added' : 'Compare'}
                    </button>
            ...
            {activeTab === 'specs' && Object.keys(product.technicalSpecsByCategory).length > 0 && (
              <div className="bg-white rounded-lg border border-gray-200 p-6">
                <SpecificationDisplay
                  ...
                  specifications={[]} // Will be populated from API
                  showSearch={true}
                  showActions={true}
                  compactView={false}
                  onExport={(format) => {
                    console.log(`Exporting specifications as ${format} for product ${product.sku}`);
                    // TODO: Implement actual export functionality
                  }}
                />
                
                {/* Fallback to legacy data while API integration is pending */}
                {Object.keys(product.technicalSpecsByCategory).length > 0 && (
                  <div className="mt-6 pt-6 border-t border-gray-200">
                    <div className="text-sm text-gray-600 mb-4">
                      📋 Legacy specification data (will be enhanced with new system)
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {Object.entries(product.technicalSpecsByCategory).map(([category, specs]) => (
