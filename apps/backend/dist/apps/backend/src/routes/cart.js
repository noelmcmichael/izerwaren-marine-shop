...
router.get('/', async (req, res) => {
    try {
        ...
        let customer = await database_1.prisma.dealer.findUnique({
            where: { id: mockCustomerId }
        });
        if (!customer) {
            customer = await database_1.prisma.dealer.create({
                data: {
                    id: mockCustomerId,
...
router.post('/items', [
    ...
    (0, express_validator_1.body)('variantId').optional().isString(),
...
router.patch('/items/:itemId', [
    (0, express_validator_1.param)('itemId').isString().notEmpty(),
...
router.post('/bulk/add', [
    ...
    (0, express_validator_1.body)('items.*.productId').isString().notEmpty(),
    (0, express_validator_1.body)('items.*.variantId').isString().notEmpty(),
    ...
    try {
        ...
        for (const item of items) {
            try {
                ...
                console.warn(`Failed to add item ${item.productId}:`, error.message);
            }
        }
        // Return updated cart
        const cartItems = await database_1.prisma.cartItem.findMany({
