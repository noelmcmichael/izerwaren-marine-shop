...
class WebhookService {
    ...
    async handleProductCreate(productData) {
        try {
            ...
            for (const variantData of productData.variants) {
                await database_1.prisma.productVariant.create({
                    data: {
            ...
            await this.logWebhookEvent('product_create', 'ERROR', null, productData.id.toString(), error.message);
    ...
    async handleProductUpdate(productData) {
        try {
            ...
            for (const variantData of productData.variants) {
                await database_1.prisma.productVariant.upsert({
            ...
            await this.logWebhookEvent('product_update', 'ERROR', null, productData.id.toString(), error.message);
            return { success: false, error: error.message };
        }
    }
    /**
     ...
     */
    async handleProductDelete(shopifyProductId) {
        try {
            ...
            await this.logWebhookEvent('product_delete', 'ERROR', null, shopifyProductId.toString(), error.message);
            return { success: false, error: error.message };
        }
    }
    /**
    ...
    async handleInventoryUpdate(inventoryData) {
        try {
            ...
            await this.logWebhookEvent('inventory_update', 'ERROR', null, inventoryData.inventory_item_id.toString(), error.message);
            return { success: false, error: error.message };
        }
    }
    /**
     ...
     */
    async logWebhookEvent(operation, status, productId, shopifyId, errorMessage) {
        try {
            ...
            await database_1.prisma.productSyncLog.create({
                data: {
